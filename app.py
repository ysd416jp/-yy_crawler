...
def get_sheet():
    ...
    # 1. 文字列を取得し、非ASCII文字を徹底的に無視して抽出
    encoded_raw = st.secrets["ENCODED_JSON"]
    # ASCII以外の文字を無視して bytes 型に変換
    encoded_bytes = encoded_raw.encode("ascii", "ignore")
    
    # 2. デコードして JSON 復元
    decoded_creds = base64.b64decode(encoded_bytes).decode("utf-8")
